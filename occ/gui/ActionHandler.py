    ...
    def _handle_action(self, action):
        ...
        if action == Actions.APPLICATION_INITIALIZE:
            ...
            for action in (
                ...
                Actions.TYPES_WINDOW_DISPLAY, Actions.FLOW_GRAPH_SEND_TO_CAMERA,
        ...
        ##################################################
            ...
            #otherwise try to save
                ...
                try:
                    ...
                    self.get_flow_graph().occ_file_path = self.get_page().get_file_path()
        ...
        elif action == Actions.FLOW_GRAPH_SEND_TO_CAMERA:
            #ocvc_send_script = os.path.dirname(self.get_page().get_file_path())+"/ocvc_send.sh"
            #top_block_file_path = os.path.dirname(self.get_page().get_file_path())+"/top_block.py"
            #ocvc_send_script = self.get_page().get_file_path+"/ocvc_send.sh"
            #top_block_file_path = self.get_page().get_file_path+"/top_block.py"
            scp_command = "scp "+os.path.dirname(self.get_page().get_file_path())+"/top_block.py"+" root@192.168.1.3:/home/root/"+"top_block.py"
            ssh_command = "ssh root@192.168.1.3 nohup python /home/root/top_block.py"
            #command = '{0} {1}'.format(ocvc_send_script, top_block_file_path)
            Messages.send_to_camera(os.path.dirname(self.get_page().get_file_path())+"/top_block.py")
            call(scp_command.split(), shell=False)
            Messages.send_to_camera_exec("/home/root/top_block.py")
            call(ssh_command.split(), shell=False)
            #subprocess.check_output([self.get_page().get_file_path,"ocvc_send.sh"])
        ##################################################
        # Gen/Exec/Stop
        ##################################################
            ...
            for b in self.get_flow_graph().get_selected_blocks():
                if b._occ_source:
                    self.main_window.new_page(b._occ_source, show=True);
