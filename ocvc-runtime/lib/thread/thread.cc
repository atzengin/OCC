...
namespace oc {
  namespace thread {
    ...
    void
    thread_bind_to_processor(oc_thread_t thread, int n)
    {
      std::vector<int> mask(1, n);
    ...
    void
    thread_bind_to_processor(oc_thread_t thread, const std::vector<int> &mask)
    {
      //DWORD_PTR mask = (1 << n);
    ...
    void
    thread_unbind(oc_thread_t thread)
    {
    ...
    int 
    thread_priority(oc_thread_t thread)
    {
    ...
    int 
    set_thread_priority(oc_thread_t thread, int priority)
    {
      // Not implemented on Windows
      return -1;
    }
