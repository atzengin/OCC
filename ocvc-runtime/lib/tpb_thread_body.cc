...
namespace oc {
    ...
    : d_exec(block, max_noutput_items)
  {
    ...
    d->threaded = true;
    d->thread = oc::thread::get_current_thread_id();
    ...
    if(block->processor_affinity().size() > 0) {
      oc::thread::thread_bind_to_processor(d->thread, block->processor_affinity());
    }

    // Set thread priority if it was set before fg was started
    if(block->thread_priority() > 0) {
      oc::thread::set_thread_priority(d->thread, block->thread_priority());
    }
    ...
    while(1) {
      ...
      {
        oc::thread::scoped_lock guard(d->d_tpb.mutex);
