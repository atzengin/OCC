...
* Author: Mark Plett
* Description:
*   SWIG interface generator file for oc_logger module.  oc_logger wraps log4cpp logging
*   for ocvc.
*******************************************************************************/
...
%include "exception.i"
%import "ocvc.i"			// the common stuff
...
%rename(logger) oc::logger;
%rename(logger_config) oc_logger_config;
%rename(logger_get_names) oc_logger_get_logger_names;
%rename(logger_reset_config) oc_logger_reset_config;

void oc_logger_config(const std::string config_filename,unsigned int watch_period = 0);
std::vector<std::string> oc_logger_get_logger_names(void);
void oc_logger_reset_config(void);
